        ...
        public static PmxModel Create([NotNull] Avatar avatar, [NotNull] Mesh mesh) {
            ...
            for (var i = 0; i < vertexCount; ++i) {
                ...
                for (var j = 0; j < affectiveInfluenceCount; ++j) {
                    var boneId = mesh.BoneNameHashes[skin[j].BoneIndex];
                    var realBoneIndex = avatar.AvatarSkeleton.NodeIDs.FindIndex(boneId);
            ...
            var indicies = new int[mesh.Indices.Count];

            for (var i = 0; i < indicies.Length; ++i) {
                indicies[i] = unchecked((int)mesh.Indices[i]);
            }

            model.FaceTriangles = indicies;

            var boneCount = avatar.AvatarSkeleton.NodeIDs.Length;
            var bones = new PmxBone[boneCount];

            var hierachy = BoneUtils.BuildBoneHierarchy(avatar);

            for (var i = 0; i < bones.Length; ++i) {
                var bone = new PmxBone();
                var transform = avatar.AvatarSkeletonPose.Transforms[i];
            ...
            var materialCount = mesh.SubMeshes.Count;
            ...
            for (var i = 0; i < materialCount; ++i) {
                ...
                material.AppliedFaceVertexCount = (int)mesh.SubMeshes[i].IndexCount;
